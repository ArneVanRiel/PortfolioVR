def OntbrekendeData():
    ...
    for data in aandelen_data_tickers:
        ...
        NetIncomeLossNecessaryData = last_period_end_date - pd.DateOffset(months=44*3) + pd.DateOffset(days=30)
        ...
        if checkperiod_end_dateNecessaryData['period_end_date'].count() == 52:
            ...
            print("check 4: " + str(52 - check11YearPeriod['period_end_date'].count()) + " ontbrekende period_end_date, " + str(check11YearPeriod['period_end_date'].count() / 52 * 100) + " %")
            print('nog uit te zoeken')
        ...
        if checkNetIncomeLossNecessaryData['NetIncomeLoss'].count() == 44:
            ...
            totalMissingData = totalMissingData + (44 - checkNetIncomeLossNecessaryData['NetIncomeLoss'].count())
            #print("check 6: " + str(44 - check11YearPeriod['NetIncomeLoss'].count()) + " ontbrekende NetIncomeLoss, " + str(check11YearPeriod['NetIncomeLoss'].count() / 44 * 100) + " %")
        ...
        if totalMissingData == 0:
            ...
            print(ticker + ' is not added: totaal ontbrekende data: ' + str(totalMissingData) + " van 245, " + str((245 - totalMissingData) / 245 * 100) + " %" + " compleet")


        totalMissingDataCombo = pd.concat([totalMissingDataCombo, ontbrekendeLiabilitiesCurrentData, ontbrekendeLiabilitiesData, ontbrekendeStockholdersEquityData, ontbrekendeNetIncomeLossData, ontbrekendeNetCashProvidedByUsedInOperatingActivitiesData, ontbrekendePurchasesOfPropertyAndEquipmentData])


    totalMissingDataGroup = totalMissingDataCombo.groupby('ticker')['ontbrekende data'].count().reset_index()
    totalMissingDataGroup['%'] = (totalMissingDataGroup['ontbrekende data'] / 320).round(2)
    ...
    print(totalMissingDataCombo)
    print(totalMissingDataGroup)
    #print('tickers in database: ' + str(aandelen_data_tickers))
    #print('tickers in niet voldoen aan snelle check: ' + str(aandelen_data_tickers_unwanted))
    #print('tickers die volledig zijn: ' + str(tickersWithFullData))

    cnxn.close()

    return totalMissingDataCombo, totalMissingDataGroup, tickersWithFullData
